...
sc_status="down"; sc_code=0
relay_status="down"; relay_code=0
pc_status="down"; pc_code=0
...
# SwarmClaw
sc_code=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:3456/health 2>/dev/null)
[ "$sc_code" = "200" ] && sc_status="up"
# Fallback: check if systemd service is active
if [ "$sc_status" = "down" ]; then
  systemctl is-active --quiet swarmclaw && sc_status="up" && sc_code=200
fi

# AI Relay (claude-relay → GPT-4o)
relay_code=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://127.0.0.1:19876/ 2>/dev/null)
[ "$relay_code" = "200" ] || [ "$relay_code" = "405" ] && relay_status="up"
if [ "$relay_status" = "down" ]; then
  systemctl is-active --quiet claude-relay && relay_status="up" && relay_code=200
fi
...
cat > "$OUT" << EOF
{
  ...
  "services": {
    ...
    "swarmclaw": {"status":"$sc_status","http":$sc_code,"port":3456},
    "claude-relay": {"status":"$relay_status","http":$relay_code,"port":19876},
